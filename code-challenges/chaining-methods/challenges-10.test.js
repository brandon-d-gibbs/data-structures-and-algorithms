...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const newArr = [];
  input.forEach(array => {
    const newArray1 = array.filter(value => (typeof value === 'number' && value%5===0)).map(value => Math.pow(2, value));
    newArr.push(newArray1);
  });
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  (char.gender === 'male' || char.gender === 'female')).map(char => char.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  const shorty = data.reduce((acc, val) => {
    if(parseInt(val.height) < acc) {
      acc = val.height;
    }
    return acc;
  },999);
  return data.filter(char => char.height === shorty)[0].name;
};
