...
const allHappy = (arr) => {
  // Solution code here...
  let ifYoureHappyAndYouKnowIt = true;
  arr.forEach(val => {
    if(!ifYoureHappyAndYouKnowIt || !val.includes(':)')){
        ifYoureHappyAndYouKnowIt = false;
    }
  });
  return ifYoureHappyAndYouKnowIt;
};
...
const findEvery = (arr, target) => {
  ...
  let foundAgain = true;
  arr.forEach(str => {
    !str.includes(target) ? foundAgain = false : foundAgain;
  });
  return foundAgain;
};
