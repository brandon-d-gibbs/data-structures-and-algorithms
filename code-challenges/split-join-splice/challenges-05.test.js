...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(line =>{
    ...
    let firstCut = line.slice(space1 + 1, line.length);
    let space2 = firstCut.indexOf(' ');
    let purchaseThis = firstCut.slice(space2 + 1, line.length);

    result.push(purchaseThis);
